...
def play_many_games_semisupervised(num_agents, num_episodes, inner_speech, learning_rate, punishment_weight, punishment_talk, agents_memory, replay, beta_1, p_peek, num_choose_act):
    ...
    for e in range(episodes):
        ...
        state1, state2, _, _ = env.step(0, 0, 0, my_sample) 
        ...
        if random.random() < p_peek: # peek another's action 20% of times: hindsight 
    ...
    for n in num_agents:
        for v in num_ent:
            for j in range(samples):
                cond_list.append([superv, n, v])
            
supervision_rate = cond_list[trial_index][0]
n = cond_list[trial_index][1]
v = cond_list[trial_index][2]
d = play_many_games_semisupervised(n, 60000, 0, 0.0001, 4, 0, 10, 10, 0.3, supervision_rate, v)
