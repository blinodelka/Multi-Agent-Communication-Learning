...
class Two_Roles_Game_Many_Agents():
    ...
    def __init__(
        ...
        for i in range(num_agents):
            previous_actions.append(collections.deque(maxlen=mean_sample)) 
        for i in range(num_agents):
...
def play_many_games_semisupervised(num_agents, num_episodes, inner_speech, learning_rate, punishment_weight, punishment_talk, agents_memory, replay, beta_1, p_peek, verbose=False):
    ...
    for e in range(episodes):
        ...
        score1 += reward1 # reward independent of second agent actions (punishment for overusing messages or actions)
        ...
        if len(agent1.memory) >= replay and len(agent2.memory) >= replay: 
            ...
            print("replay bigger than memory")
...
inn_sp = cond_list[trial_index][1]
n = cond_list[trial_index][1]
d = play_many_games_semisupervised(2, 60000, inn_sp, 0.0001, 4, 0, 10, 10, 0.3, supervision_rate)
